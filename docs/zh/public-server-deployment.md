...
推荐优先使用 `quic://...`，在 UDP 被运营商或公司网络阻断时切换到 `https://...`。
...
推荐新服务器直接用一键脚本。脚本会安装依赖、安装 Rust、构建 relay、创建 `securechat` 系统用户、签发 Let's Encrypt 证书、写入 systemd 配置、开启防火墙、安装证书续期 hook，并把后续管理入口安装为：
...
如果你已经手动准备好了 `/etc/secure-chat/tls/fullchain.pem` 和 `/etc/secure-chat/tls/privkey.pem`，可以跳过 Certbot：
