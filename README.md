...
Rust relay 服务器、原生 macOS SwiftUI 客户端、原生 iOS SwiftUI 客户端、
原生 Android 客户端和 Windows WinUI 3 客户端；各端通过同一套 Rust FFI 运行时
和 relay 协议互通。
...
- 设备注册、发送、拉取、送达/已读回执、P2P 和 APNs push 操作都使用
  Ed25519 relay request signature。
- relay 使用 SQLite 持久化公开 pre-key bundle、离线密文队列、回执、
  P2P candidate 和 APNs device token。
- macOS/iOS 使用 Apple Keychain 保存身份密钥和本地存储密钥。
- Windows 使用 Windows Credential Manager/DPAPI 保存身份密钥和本地存储密钥，
  数据目录位于 `%LOCALAPPDATA%\SecureChat`。
...
- 本地 SQLite 保存联系人、加密 ratchet session、加密消息正文和缓存的 relay
  密文。
- macOS、iOS、Android 和 Windows 客户端支持登录、联系人、邀请导入/复制、聊天记录、
  relay 设置、临时会话、后台轮询、通知和 sent/delivered/read 状态展示。
- v0.2.5 起 macOS/iOS/Android 支持完整聊天能力；v0.2.6 起 Windows 也对齐：
  联系人昵称编辑和本地强删除、文件/图片发送、表情图片导入与发送、Unicode
  emoji 输入显示、阅后即焚消息，以及类似主流聊天软件的“贴底才自动滚动”行为。
...
在 Windows 10 22H2+ 或 Windows 11 上构建 WinUI 3 客户端：

```powershell
.\script\build_windows.ps1 -Configuration Release
```

脚本会构建 `secure_chat_ffi.dll`、复制到 Windows app 的 `Native` 目录，并生成
签名 MSIX。没有正式代码签名证书时，脚本会创建本地测试证书；公开分发需要替换为
可信代码签名证书。
...
产物会写到 `dist/SecureChatMac-0.2.7.dmg`。默认使用 ad-hoc 签名，适合本地
...
发布自动化会把最终可安装 APK 复制为 `dist/SecureChatAndroid-0.2.7.apk`，并和
...
```powershell
.\script\build_windows.ps1 -Configuration Release -Version 0.2.7.0 -ArtifactVersion 0.2.7
```

产物会写到 `dist/SecureChatWindows-0.2.7.msix`，并生成
`dist/SecureChatWindows-0.2.7.msix.sha256`。测试证书公钥会导出为
`dist/SecureChatWindows-TemporaryKey.cer`，用于本地安装验证。
...
- v0.2.7 发布说明：[docs/releases/v0.2.7.md](docs/releases/v0.2.7.md)
...
- 一对一聊天和临时会话都使用同一套版本化内容载荷，旧文本消息仍通过
  `body` fallback 显示。
...
- `crates/secure-chat-core`：身份密钥、pre-key bundle、邀请链接、
  X3DH-style session setup、Double Ratchet、安全码、relay API 类型和 padded
  transport frame。
...
- `crates/secure-chat-desktop`：面向桌面和移动 FFI 的 runtime，包含 Keychain
  身份存储、SQLite 联系人、加密 session、加密消息、附件/表情本地索引、
  阅后即焚状态和远端消息 ID。
...
- Attachments：文件、图片和表情项会记录 metadata、sha256、本地路径和传输状态。
  relay 仍只看到 Double Ratchet 加密后的不透明密文分片。
...
- 群组功能已从 macOS、iOS、Android、Windows 客户端和公开 FFI 调用面移除。
  仓库中历史 group prototype/relay MLS endpoint 仅作为未暴露实验代码保留，
  当前版本不把群聊列为支持能力。
